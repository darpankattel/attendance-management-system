...
ImageUploadView::ImageUploadView(wxPanel *parent, wxString title, wxPoint pos, wxSize size, wxString& defaultImage, wxBitmapType type, wxString tooltip) : ImageView(parent, pos, size, defaultImage, type)
{
    if (title.IsEmpty())
...
{
    wxFileDialog *openFileDialog = new wxFileDialog(this, wxString("Choose a Image"), wxEmptyString, wxEmptyString, wxString("Image Files (*.png;*.jpg;)|*.png;*.jpg;"), wxFD_OPEN | wxFD_FILE_MUST_EXIST);
    if (openFileDialog->ShowModal() == wxID_CANCEL)
        return;
    this->imagePath = openFileDialog->GetPath();
    std::cout<<"imagePath: "<<this->imagePath<<std::endl;
    ImageView::ChangeImage(this->imagePath, wxBITMAP_TYPE_ANY);
...
std::string ImageUploadView::getImageFullPath()
{
    return this->imagePath.ToStdString();
}